...
from .dgcnn_utils import get_graph_feature
import time

def check_mem(str_):
    time.sleep(1)
    print("checking mem at",str_)
...
class VNDGCNN(nn.Module):
    ...
    def forward(self, x, list):
        ...
        # check_mem('graph featre 1')
        # print(x.shape)
        x = self.conv1(x)
        x1 = self.pool1(x)
        
        x = get_graph_feature(x1, k=self.n_knn)
        # check_mem('graph featre 2')
        x = self.conv2(x)
        x2 = self.pool2(x)
        ...
        x = self.conv3(x)
        x3 = self.pool3(x)
        # check_mem('graph featre 3 after pool')
        
        x = get_graph_feature(x3, k=self.n_knn)
        x = self.conv4(x)
        x4 = self.pool4(x)
        # check_mem('graph featre 4 after pool')
